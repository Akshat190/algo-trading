...
def automated_sharekhan_login(api_key, secret_key, username, password, totp_secret, vendor_key="", version_id=""):
    ...
    try:
        ...
        # Step 1: Get the login page and open browser for manual flow
        print("\n1. Accessing login page...")
        login_page_response = session.get(login_url)
        try:
            print("Opening login URL in your default browser. Complete login and OTP there if prompted.")
        ...
        # Step 3: Submitting login credentials (best-effort; some flows require full browser login)
        ...
        try:
            ...
            print(f"Login JSON keys: {list(login_json.keys())}")
        except Exception:
            pass
        
        # Check if TOTP is required
        needs_totp = any(keyword in login_response.text.lower() for keyword in ['otp', 'authentication', 'verify', 'token']) or (isinstance(login_json, dict) and (login_json.get('statusCode') in [200, 202, 401, 403] or login_json.get('totpStatus') is not None))
        
        if needs_totp or login_response.status_code in [302, 401, 403]:
            print("\n4. TOTP verification required...")
            
            # Generate or input TOTP code
            if totp_secret:
                totp_code = generate_totp_code(totp_secret)
            else:
                try:
                    user_input = input("Enter your 6-digit TOTP from authenticator: ").strip()
                except Exception:
                    user_input = ""
                if not (len(user_input) == 6 and user_input.isdigit()):
                    print("Invalid TOTP. Please enter a 6-digit code.")
                    return None
                totp_code = user_input
            
            if not totp_code:
                print("Failed to obtain TOTP code")
                return None
            ...
            # Build TOTP payload using fields from login JSON if present
            request_session_id = None
            if isinstance(login_json, dict):
                request_session_id = login_json.get('requestSessionId') or login_json.get('request_session_id')
            totp_payload = {
                'apiKey': api_key,
                'otp': totp_code
            }
            if request_session_id:
                totp_payload['requestSessionId'] = request_session_id
            ...
            for endpoint in totp_endpoints:
                try:
                    ...
                    try:
                        print(f"TOTP resp status: {totp_response.status_code}, body: {totp_response.text[:200]}")
                    except Exception:
                        pass
        ...
        # Step 5: Extract or prompt for request_token
        print("\n5. Extracting request_token...")
        request_token = extract_request_token_from_response(final_response)
        if not request_token:
            # Ask user to paste token from browser redirect
            try:
                pasted = input("Paste request_token from the redirected browser URL (or press Enter to skip): ").strip()
            except Exception:
                pasted = ""
            if pasted:
        ...
        if not request_token:
            print("Failed to obtain request_token. Complete login in browser and paste the token when asked.")
            return None
        ...
        try:
            ...
            if isinstance(access_response, dict):
                ...
                # Some responses nest the token under data.token
                if not access_token and isinstance(access_response.get('data'), dict):
                    data_obj = access_response.get('data')
                    access_token = data_obj.get('token') or data_obj.get('access_token') or data_obj.get('accessToken')
            else:
...
def main():
    ...
    # CREDENTIALS
    username = "4031914"            # Your Sharekhan username/customer id
    password = "Kb@123456"          # Your Sharekhan password  
